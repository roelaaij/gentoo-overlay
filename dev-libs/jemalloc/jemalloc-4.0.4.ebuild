...
multilib_src_install() {
	...
	IFS='_' read -ra ABIARR <<< "${MULTILIB_ABI_FLAG}"
	rm "${D}/usr/bin/jeprof"
	cp "${BUILD_DIR}/bin/jeprof" "${D}/usr/bin/jeprof-${ABIARR[2]}" || die
}
...
src_install() {
	...
	mv "${D}/usr/bin/jeprof-64" "${D}/usr/bin/jeprof" || die
	chmod 755 "${D}/usr/bin/jeprof" || die
	rm "${D}/usr/bin/jeprof-32" || die
}
