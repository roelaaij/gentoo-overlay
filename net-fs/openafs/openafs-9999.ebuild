...
src_unpack() {
	unpack ${A}
	git-r3_src_unpack
	rm "${WORKDIR}/gentoo/patches/000_all_p20181001.patch" || die
	cp "${FILESDIR}/001_all_compiler-settings.patch" "${WORKDIR}/gentoo/patches" || die
	rm -rf "${WORKDIR}/${P}/doc" || die
	mv "${WORKDIR}/${MY_P}/doc" "${WORKDIR}/${P}/doc" || die
}
