...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-1.13.1-install-dirs.patch
	"${FILESDIR}"/${PN}-1.12.0-glog-0.6.0.patch
	"${FILESDIR}"/${PN}-1.12.0-clang.patch
	"${FILESDIR}"/${PN}-1.12.0-trivially-copyable.patch
	"${FILESDIR}"/${PN}-1.13.1-fmt-9.patch
	"${FILESDIR}"/${PN}-1.13.1-cuda12_{1,2,3,4}.patch
)
...
src_configure() {
	...
	if use cuda && [[ -z ${TORCH_CUDA_ARCH_LIST} ]]; then
		mycmakeargs+=( -DTORCH_CUDA_ARCH_LIST="${TORCH_CUDA_ARCH_LIST}" )
	fi
