...
PATCHES=( "${FILESDIR}/${PN}-2.10-cuda-host-compiler.patch"
		  "${FILESDIR}/${PN}-2.10-sparse-transpose-op.patch")
...
src_configure() {
	...
	do_configure() {
		...
		if use cuda; then
			...
			export GCC_HOST_COMPILER_PATH="/usr/x86_64-pc-linux-gnu/gcc-bin/12/gcc"
			export CUDA_HOST_COMPILER_PATH="/usr/x86_64-pc-linux-gnu/gcc-bin/11/gcc"
			...
			if [[ $(cuda-config -s) != *$(gcc-version)* ]]; then
				...
				ewarn "instead be compiled using: ${CUDA_HOST_COMPILER_PATH}."
