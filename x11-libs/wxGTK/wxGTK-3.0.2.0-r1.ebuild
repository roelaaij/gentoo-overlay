...
RDEPEND="
	...
	X?  (
		...
		>=x11-libs/gtk+-3.10.0:3[${MULTILIB_USEDEP}]
		>=x11-libs/gdk-pixbuf-2.27.1:2[${MULTILIB_USEDEP}]
		x11-libs/libSM[${MULTILIB_USEDEP}]
		x11-libs/libXxf86vm[${MULTILIB_USEDEP}]
		>=x11-libs/pango-1.32.4[X,${MULTILIB_USEDEP}]
		gstreamer? (
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		...
		dosym /usr/$(get_libdir)/wx/config/gtk3-unicode-${SLOT} /usr/bin/wx-config-${SLOT}-${ABI}
	fi
}
